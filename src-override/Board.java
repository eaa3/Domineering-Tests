...
public abstract class Board<Move> {
	...
	public GameTree<Move> tree() {
		if (availableMoves().isEmpty()) return new GameTree<Move>(this, new LinkedHashMap<Move, GameTree<Move>>(), value());
		else return (nextPlayer() == Player.MAXIMIZER ? maxTree() : minTree());
	}
	...
	public GameTree<Move> maxTree() {
		...
		LinkedHashMap<Move, GameTree<Move>> children = new LinkedHashMap<Move, GameTree<Move>>();
